...
1. **MCP Server** (`src/server.ts`) — Registers MCP Tools. Tools execute actions (e.g., spawning a `git commit` subprocess).
...
3. **Security Hook** (`src/hooks/secret-guard.ts`) — A `BeforeTool` hook that intercepts every MCP tool call. It scans arguments for sensitive file patterns (`.env`, `.pem`, `.key`, `credentials.json`, `id_rsa`, etc.) and returns `{decision: 'deny'}` if found. Exit code 2 is a fail-safe for internal errors.
...
3. Register any tools in `src/server.ts`, then rebuild.
