...
本擴充功能將 Agent 從通用助手轉變為專業工程師，嚴格遵循 **感知-推理-行動 (Perceive-Reason-Act)** 的循環：

1.  **感知 (Perceive)**：根據定義的標準分析使用者的專案環境。
2.  **推理 (Reason)**：參考內部知識庫 (`SKILL.md`) 制定策略。
3.  **行動 (Act)**：使用預先驗證的模版與腳本執行精確的工作流程。
...
2.  **指令 (Commands)**：在 `commands/neo/` 中使用 TOML 格式定義新的工作流程。
